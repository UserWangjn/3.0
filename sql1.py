...
a=2      #0用户体验   1、业务分析  2apm
i=1       #第几个监控对象
n=2
top_level_parent_id=['147','112','147','112']   #'监控目标顶级节点ID，如果是Server则保存业务系统ID，browser则保存应用ID，Browser的模块则保存模块ID，业务则保存业务的ID',
parent_id=['147','112','132','132']
target_id=['147','112','132','6155']
target_name=['test-alarm0806','test-alarm-report0726','test-alarm-report0728','URI/portal/orderStatus']
agreement_id='1998'
alarm_type='1' #1策略告警    2只能告警
alarm_settings_id=['189','105','108','188']
policy_id=['196','721','721','209']

ts=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
# ts='2018-08-11 18:22:30'
...
for e in range(1):
     ...
     sqls="INSERT INTO NL_U_ALARM_EVENTS_TRACE (target_type, target_children_type, top_level_parent_id, parent_id, target_id, target_name, agreement_id, status, read_status, alarm_type, alarm_settings_id, policy_id, begin_time,is_useful, message, ignore_status, sample_group, sample_group_values, ctime, mtime) " \
                                        "VALUES ("+target_type[a]+",'"+target_children_type[a][i]+"', '"+top_level_parent_id[n]+"', '"+parent_id[n]+"', '"+target_id[n]+"', '"+target_name[n]+"', '"+agreement_id+"', '1', '0', '"+alarm_type+"', '"+alarm_settings_id[n]+"', '"+policy_id[n]+"', '"+ts+"', '0', '', '0', '1', '', '"+ts+"', '"+ts+"')"
...
"VALUES ('"+data1(sqls)+"', '0', '1', '1', '1','"+ts+"','"+ts+"')"
     ...
     metric_Aapplication=['d','er','epm','qps','ngcpm','ngtpm','jgcpm','jgtpm','tc','sht']   #应用指标/模块/主机
     ...
     Cril_rate=[5,6,7]
     t=data1(sqls)
     for i in range(len(metric_Aapplication)):
          ...
          "VALUES ('"+t+"', '"+metric_Aapplication[i]+"', '2', '10', '100', '1', '1', '"+ts+"')"
